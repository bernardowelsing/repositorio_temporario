...
func = {}
Tunnel.bindInterface("ve_roubojewelry",func)
...
function func.callPolice(x,y,z)
	local source = source
	TriggerClientEvent("vrp_sound:fixed",-1,source,x,y,z,100,'alarm',0.4)
	local policia = vRP.getUsersByPermission("policia.permissao")
	for l,w in pairs(policia) do
		...
		if player then
			async(function()
				...
				TriggerClientEvent('chatMessage',player,"Dispatch",{64,130,255},"O roubo começou na ^1Joalheria^0, dirija-se até o local e intercepte o assaltante.")
				--TriggerClientEvent("NotifyPush",player,{ code = 35, title = "O roubo começou na Joalheria, dirija-se até o local e intercepte o assaltante.", x = x, y = y, z = z, badge = "Joalheria." })
	...
	if user_id then
		if timers[id] == 0 or not timers[id] then
			...
			SetTimeout(10000,function()
				vRPclient.setStandBY(source,parseInt(600))
				vRP.giveInventoryItem(user_id,jewels[math.random(5)].item,math.random(40,60))
			end)
		else
