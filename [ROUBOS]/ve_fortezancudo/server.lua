...
local roubositemlist = {
	[1] = { ['place'] = "Forte Zancudo", ['seconds'] = 240, ['rewmin'] = 900000, ['rewmax'] = 1200000 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECAR POLICIA
-----------------------------------------------------------------------------------------------------------------------------------------
function pcy.checarPolicia()
	...
	if user_id then
		...
		elseif (os.time()-timedown) <= 28800 then
			TriggerClientEvent("Notify",source,"aviso","Os cofres estão vazios, aguarde <b>"..vRP.format(parseInt((5000-(os.time()-timedown)))).." segundos</b> até que os civis efetuem depositos.",8000)
			return false
		end
	end
	return true
